...
- Android: JVM unit tests in `FBAudio-Android/app/src/test/java/com/dharmachakra/fba_android/`,
...
- **Playback (Android)**: `PlaybackService` is a Media3 `MediaLibraryService`. `PlaybackResolver` (service-side, Hilt singleton) turns a catNum into the chapter queue (stream or downloaded file, remastered/original) and computes the resume point (account checkpoint wins, else `player_prefs`); `PlaybackPersistence` listens to the player and writes progress (prefs, `/api/v1/checkpoints/`, Recently listened) — so Auto, the notification and `PlayerViewModel` share one queue and state. `PlayerViewModel` only drives the `MediaController`. **Android Auto**: `LibraryTree` builds the browse tree (ids in `MediaIds`, unit-tested): Recent · Downloads · Sangharakshita (series → talks) · Latest, plus search via `TalkRepository.searchAudio`; `res/xml/automotive_app_desc.xml` + manifest meta-data declare the app. On-device test `PlaybackServiceBrowseTest` (`./gradlew connectedDebugAndroidTest`) walks the tree with a `MediaBrowser`. CarPlay is pending Apple's entitlement.
...
- **FBA `/api/v2`** (in progress with the FBA web team; launch target 6 Dec 2026) will replace the scraping layer. Planning docs are kept out of this public repo (gitignored under `docs/`); ask David for them.
